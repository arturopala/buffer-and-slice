...
abstract class MappedArraySlice[T] private (fromIndex: Int, toIndex: Int) extends Slice[T] {
  ...
  final def map[K](f: T => K): Slice[K] = MappedArraySlice.lazyMapped[K, A](fromIndex, toIndex, array, mapF.andThen(f))
  ...
  final def slice(from: Int, to: Int): Slice[T] = {
    ...
    if (f == 0 && t == length) this
      ...
      MappedArraySlice.lazyMapped[T, A](fromIndex + f, fromIndex + t, array, mapF)
  }
  ...
  final def take(n: Int): Slice[T] =
    MappedArraySlice.lazyMapped[T, A](fromIndex, Math.min(fromIndex + Math.max(0, n), toIndex), array, mapF)

  /** Lazily narrows Slice to last N items. */
  final def takeRight(n: Int): Slice[T] =
    MappedArraySlice.lazyMapped[T, A](Math.max(toIndex - Math.max(0, n), fromIndex), toIndex, array, mapF)

  /** Lazily narrows Slice to exclude first N items. */
  final def drop(n: Int): Slice[T] =
    MappedArraySlice.lazyMapped[T, A](Math.min(fromIndex + Math.max(0, n), toIndex), toIndex, array, mapF)

  /** Lazily narrows Slice to exclude last N items. */
  final def dropRight(n: Int): Slice[T] =
    MappedArraySlice.lazyMapped[T, A](fromIndex, Math.max(toIndex - Math.max(0, n), fromIndex), array, mapF)
